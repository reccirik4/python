...
# [YENİ] Ses kütüphanesi
from gui.voice_library_dialog import VoiceLibrary, VoiceLibraryDialog
...
class MainWindow(QMainWindow):
    ...
    _sinyal_log = pyqtSignal(str, str)
    _sinyal_ilerleme = pyqtSignal(float, str)
    _sinyal_bitti = pyqtSignal(bool)
    _sinyal_onizleme_log = pyqtSignal(str, str)
    _sinyal_onizleme_oynat = pyqtSignal(str, str, str, int)
    _sinyal_zamanlama_guncelle = pyqtSignal(dict)
    ...
    def __init__(self, config: ConfigManager):
        ...
        self._seslendirme_thread = None

        # [YENİ] Ses kütüphanesi — proje dizininde dubsync_voices/ klasörü
        proje_dizin = str(Path(__file__).resolve().parent.parent)
        self._library = VoiceLibrary(proje_dizin)
        ...
        # [YENİ] Karakter paneline kütüphaneyi ata
        self._karakter_panel.library_ayarla(self._library)
        ...
        self._sinyal_zamanlama_guncelle.connect(self._altyazi_tablosu.zamanlama_guncelle)

        logger.info("Ana pencere oluşturuldu.")
        logger.info(
            "Ses kütüphanesi: %s (%d ses)",
            self._library.voices_klasor,
            self._library.ses_sayisi,
        )
    ...
    def _arayuz_olustur(self):
        ...
        ana_layout.addWidget(self._dosya_seciciler_olustur())
        ana_layout.addWidget(self._ana_icerik_olustur(), stretch=1)
        ana_layout.addWidget(self._kontrol_paneli_olustur())
        ana_layout.addWidget(self._ilerleme_olustur())
        ana_layout.addWidget(self._log_alani_olustur())
    ...
    def _kontrol_paneli_olustur(self) -> QWidget:
        ...
        self._btn_baslat.setStyleSheet(
            ...
            "font-weight: bold; font-size: 14px; border-radius: 6px; padding: 6px 20px; }"
    ...
    def _ilerleme_olustur(self) -> QWidget:
        ...
        self._lbl_ilerleme_yuzde.setStyleSheet("font-size: 12px; font-weight: bold; color: #333;")
    ...
    def _menu_olustur(self):
        ...
        # [YENİ] Ses kütüphanesi
        act_ses_kutuphanesi = QAction("🎙 Ses Kütüphanesi...", self)
        act_ses_kutuphanesi.setShortcut("Ctrl+L")
        act_ses_kutuphanesi.triggered.connect(self._ses_kutuphanesini_goster)
        araclar_menu.addAction(act_ses_kutuphanesi)
    ...
    def _srt_dosya_sec(self):
        ...
        yol, _ = QFileDialog.getOpenFileName(
            self, "Altyazı Dosyası Seç", son_klasor,
    ...
    def _video_dosya_sec(self):
        ...
        yol, _ = QFileDialog.getOpenFileName(
            self, "Video Dosyası Seç", son_klasor,
    ...
    def _srt_yukle_ve_goster(self, yol: str):
        try:
            ...
            # [YENİ] library ile birlikte yükle
    ...
    def _seslendirme_iptal(self):
        cevap = QMessageBox.question(
            self, "İptal", "Seslendirme iptal edilsin mi?",
    ...
    def _seslendirme_pipeline(self):
        ...
        cikis_ana = os.path.join(str(Path(video_yolu).parent), "dubsync_output")
        ...
        try:
            ...
            self._sinyal_log.emit(f"  Motorlar hazır: {', '.join(kaydedilenler)}", "success")
            ...
            def ilerleme_callback(ilerleme):
                yuzde = 5 + (ilerleme.yuzde * 0.50)
                satir = ilerleme.mevcut_satir
                metin_kisa = satir.temiz_metin[:40] if satir and satir.temiz_metin else ""
            ...
            sonuclar = loop.run_until_complete(
                self._tts_manager.toplu_uret(dosya, segment_klasoru, ilerleme_callback)
            )
            ...
            if basarili == 0:
                self._sinyal_log.emit("Hiçbir ses üretilemedi! İşlem durduruluyor.", "error")
                self._sinyal_bitti.emit(False)
                return
            ...
            for satir, sonuc in sonuclar:
                if sonuc.basarili and sonuc.dosya_yolu:
                    ...
                    ses_sureleri[satir.sira] = sonuc.sure_ms

            rapor = analizor.toplu_analiz(dosya, ses_sureleri)
            zamanlama_sonuclari = {zs.satir_sira: zs for zs in rapor.sonuclar}
            self._sinyal_zamanlama_guncelle.emit(zamanlama_sonuclari)

            self._sinyal_log.emit(
                f"  Zamanlama: {rapor.sigiyor} sığıyor, "
                f"{rapor.hafif_hizlandir + rapor.orta_hizlandir} hızlandırılacak, "
                f"{rapor.tasma} taşma"
                + (f", {rapor.cakisma_sayisi} çakışma" if rapor.cakisma_sayisi > 0 else ""),
                "info",
            )

            # ── ADIM 4: Ses birleştirme ──
            self._sinyal_log.emit("Adım 4/6: Ses segmentleri birleştiriliyor...", "info")
            ...
            if not os.path.isfile(birlesik_ses_yolu):
                self._sinyal_log.emit("Birleşik ses dosyası oluşturulamadı!", "error")
                self._sinyal_bitti.emit(False)
                return
            ...
            ses_cikarildi = AudioDucker.videodan_ses_cikar(
                video_yolu, orijinal_ses_yolu,
                sr=config.al("ses.ornekleme_hizi", 48000)
            )
            if not ses_cikarildi:
                self._sinyal_log.emit("Videodan ses çıkarılamadı! FFmpeg kontrol edin.", "error")
                self._sinyal_bitti.emit(False)
                return
            ...
            if not os.path.isfile(mikslenmis_ses_yolu):
                self._sinyal_log.emit("Miksleme başarısız!", "error")
                self._sinyal_bitti.emit(False)
                return
            ...
            def export_ilerleme(yuzde: float):
                gercek_yuzde = 90 + (yuzde * 0.10)
                self._sinyal_ilerleme.emit(gercek_yuzde, f"Video export: %{yuzde:.0f}")

            export_sonuc = exporter.export(
                video_yolu=video_yolu,
                ses_yolu=mikslenmis_ses_yolu,
                ilerleme_callback=export_ilerleme,
            )
            ...
            self._sinyal_log.emit(f"Seslendirme pipeline hatası: {e}", "error")
            self._sinyal_bitti.emit(False)
    ...
    def _onizleme_uret(self, metin: str, motor_adi: str, ses_id: str, isim: str, veri: dict):
        ...
        gecici_dir = os.path.join(tempfile.gettempdir(), "dubsync_preview")
        ...
        def _uret_thread():
            try:
                ...
                if motor is None:
                    self._sinyal_onizleme_log.emit(f"Motor bulunamadı: {motor_adi}", "error")
                    return
                ...
                if not motor.hazir:
                    try:
                        ...
                        self._sinyal_onizleme_log.emit(
                            f"Motor başlatma hatası ({motor_adi}): {baslat_hata}", "error"
                        )
                        loop.close()
                        return

                if not motor.hazir:
                    self._sinyal_onizleme_log.emit(f"Motor başlatılamadı: {motor_adi}", "error")
                    loop.close()
                    return

                import hashlib
                dosya_hash = hashlib.md5(f"{metin}{motor_adi}{ses_id}".encode()).hexdigest()[:8]
                ...
                if motor_adi == "xtts_v2":
                    # Öncelik: klon_yolu > ses_id (dosya yolu olabilir) > hata
                    if klon_yolu and os.path.isfile(klon_yolu):
                        gercek_ses_id = klon_yolu
                    elif ses_id and os.path.isfile(ses_id):
                        gercek_ses_id = ses_id
                    else:
                        self._sinyal_onizleme_log.emit(
                ...
                if sonuc.basarili and os.path.isfile(cikis):
                    self._sinyal_onizleme_oynat.emit(cikis, isim, metin[:60], sonuc.sure_ms)
                    self._sinyal_onizleme_log.emit(
                        f"Önizleme hazır: {isim} ({sonuc.sure_ms}ms)", "success"
                    )
                else:
                    hata = sonuc.hata_mesaji if sonuc else "Bilinmeyen hata"
                    self._sinyal_onizleme_log.emit(f"Önizleme hatası: {hata}", "error")

            except Exception as e:
                self._sinyal_onizleme_log.emit(f"Önizleme hatası: {e}", "error")
        ...
        try:
            if motor_adi == "edge_tts":
                ...
                return ElevenLabsEngine({
                    ...
                    "model": self._config.al(
                        "tts_motorlari.elevenlabs.model", "eleven_multilingual_v2"
                    ),
                })
    ...
    def _filmden_klonla(self, karakter_id: str):
        ...
        if dialog.exec() == CloneDialog.DialogCode.Accepted:
            ...
            if referans and os.path.isfile(referans):
                kart = self._karakter_panel._kartlar.get(karakter_id)
                if kart:
                    # referans_ses_ayarla → _klonlama_baslat çağırır
                    kart.referans_ses_ayarla(referans)
                self.log(
                    f"Referans alındı: {karakter_id} → "
                    f"{os.path.basename(referans)} — klonlama başlıyor...",
                    "info",
                )
    ...
    def _ses_kutuphanesini_goster(self):
        """Ses kütüphanesi dialogunu açar (Araçlar menüsü, Ctrl+L)."""
        dialog = VoiceLibraryDialog(self._library, parent=self)

        def ses_secildi(isim: str, tam_yol: str):
            self._karakter_panel._tum_kartlari_yenile()
            self.log(f"Ses kütüphanesinden seçildi: {isim}", "success")

        dialog.ses_secildi.connect(ses_secildi)
        dialog.exec()
        # Dialog kapandıktan sonra kartları yenile
        self._karakter_panel._tum_kartlari_yenile()
    ...
    def _dil_degisti(self, dil_kodu: str):
        ...
        self._config.ayarla("tts_motorlari.xtts_v2.dil", dil_kodu)

        if self._tts_manager:
            xtts = self._tts_manager._motorlar.get("xtts_v2")
            if xtts and xtts.hazir:
                xtts.dil = dil_kodu
                logger.info("XTTS dili güncellendi: %s", dil_kodu)
    ...
    def _ayarlari_sifirla(self):
        cevap = QMessageBox.question(
            self, "Sıfırla", "Tüm ayarlar varsayılana döndürülsün mü?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if cevap == QMessageBox.StandardButton.Yes:
    ...
    def _hakkinda_goster(self):
        QMessageBox.about(
            ...
            "<li>Ses kütüphanesi (dubsync_voices/) ile kalıcı klon yönetimi</li>"
    ...
    def _stil_uygula(self):
        self.setStyleSheet("""
            QMainWindow { background-color: #fafafa; color: #222222; }
            ...
            QLineEdit:read-only { background-color: #f5f5f5; color: #555555; }
            QComboBox { color: #222222; background-color: #ffffff; }
            QSpinBox, QDoubleSpinBox { color: #222222; background-color: #ffffff; }
            QCheckBox { color: #333333; }
            QTabBar::tab { color: #333333; }
            QPushButton {
            ...
            QSplitter::handle { background-color: #ddd; border-radius: 2px; }
            QSplitter::handle:hover { background-color: #bbb; }
            QStatusBar { font-size: 11px; color: #666666; }
            QScrollArea { background-color: #fafafa; }
        """)
